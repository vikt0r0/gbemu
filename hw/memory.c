...
// Dump of the original Gameboy bios mapped to memory.
...
ubyte_t memory_read_byte(memory_t *mem, uword_t addr) {
	...
	// If we are accessing a low address and the bios is mapped,
	// read from the bios memory.
...
uword_t memory_read_word(memory_t *mem, uword_t addr) {
	addr = translate_address_echo(addr);
	// Should work as Gameboy as well as x86 are both little
	...
	// bulletproofed...
	uword_t a = memory_read_byte(mem, addr);
	uword_t b = memory_read_byte(mem, addr+1) << (sizeof(uword_t) / 2) * 8;
	return (a | b);
}
...
void memory_write_word(memory_t *mem, uword_t addr, uword_t data) {
	...
	// ever happen, so perhaps irrelevant...
	//addr = translate_address_echo(addr);
	ubyte_t a = data; // a = A
